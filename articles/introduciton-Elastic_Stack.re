...
世の中にはたくさんのログやデータがあります。サーバー運用など、いろんなかたちで携わっている方が多いのではないでしょうか。
...
サーバーにある大量のログファイルなどのファイルを一箇所に集約する用途で用います。
...
たとえば、サーバーのCPUや、メモリーの使用率、ディスクIOなどのデータだけでなく、プロセスなども収集できます。
...
さまざまなプロトコルに対応しているため、MySQLのクエリなどについてKibanaを用いてビジュアライズすることも可能です。
...
たとえば、Windowsサーバーを運用しており、監査目的でログオンしたユーザーを把握したい場合は、イベントIDの"ログオン: 4624"や"ログオン失敗: 4625"を指定するだけでイベントログを収集することが可能です。
...
サーバーの稼働状況を監視できます。
ICMPでサーバーの稼働状況を把握することも可能ですし、HTTPでサービス稼働も把握することが可能です。
