...
そして、ElasticsearchにインデクシングされたデータをKibanaで可視化するところまでを行いました。
...
お気付きになった方もいるかと思いますが、このまま取得したいログが増えると、どんどんif文が増えてコンフィグの可読性が悪くなってしまいます。
...
ここからは、@<code>{Multiple Pipelines}について説明していきます。


== Multiple Pipelinesについて


さきほどまでは、ひとつのファイルにパイプラインファイルを記載していました。
しかし、"Multiple Pipelines"を使用することで、データソース毎にパイプラインファイルを分けて定義することができます。
また、リソースの配分もログの種類毎にできます。

それでは、具体的にどのような設定をする必要があるかを見ていきます。
