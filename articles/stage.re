...
こういった光景、よくありますよね（筆者も昔経験したことがあります）。
...
……できるのです！
ログを取り込むための方法さえ知っていれば、さまざまなログを可視化することができます。

ただ、ログを可視化するには、取り込むログを構造化して収集する必要があります。
そこで、ここではLogstashを用いて好きなように可視化する方法を紹介します。
===[/column]

== Logstash
前述しましたが、LogstashはElastic社が提供するオープンソースログ収集管理ツールです。
...
5.10.83.30のグローバルIPはElastic社公式HPのサンプルで使用しているグローバルIPを使っています。
...
あれ？
ログが@<code>{massage}にひとかたまりで入っています……。
IPアドレス、バージョン、ステータスコードなどが別々にフィールドに入っている、つまりデータが個別に分類されている状態をつくりたかったのですが、うまくいかなかったようです。
...
では、アクセスログを適切に取り込むための方法を解説しましょう。

== Apacheのアクセスログを取り込むまでのステップ

ログを適切に取り込むには、FILTERでログフォーマットに合わせて定義をする必要があります。
ここではどのようにログを取り込むかをステップを踏んで解説していきたいと思います。
...
GrokPatternの"CISCOFW606001"に"606002"も含んでいるのですが、文字数がを短くするために"606001"に集約してます。
