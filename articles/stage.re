...
「AWSでLogstashを使ってみる」のサンプルコードをみると、Logstashのコンフィグの記述はかなり複雑だということがわかります。
...
たとえば、ソフトウェアのログ・サーバーのメトリクス情報・Webアプリケーションのデータ・データストア・さまざまなクラウドサービスなどから
データを収集できます。


=== FILTERS
INPUTしたデータソースをLogstashのFilterで解析し、構造化します。
#@#フィールドを識別するっていきなりでてくるとわかりにくい+無くても繫がるのでとりました。入れるならfieldsについて説明が欲しいかも。
...
今回は、Apacheのアクセスログと違ってすでにフィールド名が公式として定義されているので、フィールド名をそのまま使用します。
...
GrokPattenの内容を理解し、自分の意図したデータを取得できるようになりましょう。
...
AWSのサービスに対してもログを取得できるということがわかったのではないでしょうか。
