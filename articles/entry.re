...
サーバーが1台ならいいのですが、数十台になってきたら、辛さしかないです。
...
仮にWebサーバー（Nginx）を4台運用しているとします。
日々のアクセス状況を見たいため、Nginxのアクセスログ（access.log）を取得したいと思います。

 1. Webサーバーにアクセスし、access.logをscpで取得します
 1. Webサーバーが4台あるため、1を4回繰り返します
...
ザックリとした説明なので、なんとなく言っている意味がわかるレベルで読み進めてもらえればと思います。
...
大量のサーバーにあるログファイルなどのファイルを一箇所に集約する用途で用います。
...
例えば、サーバーのCPUや、メモリーの使用率からディスクIOなどのデータだけでなく、プロセスなども収集できます。
...
例えば、Windowsサーバーを運用しており、監査目的でログオンしたユーザーを把握したい場合は、イベントIDの"ログオン: 4624"や"ログオン失敗: 4625"を指定するだけで、収集することが可能です。
