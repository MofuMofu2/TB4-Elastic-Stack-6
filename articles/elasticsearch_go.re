...
次のコマンドを実行してDockerイメージを取得してください。
...
今回はgo getでインストールしますが、実際のプロダクト利用時はdepなどのパッケージ管理ツールの利用をお勧めします。
...
その前に、検索するデータを投入するためのIndexとTypeを作成します。
...
また、Mapping定義を作成することにより各フィールド単位でより細かな検索設定をおこなうことが可能なため、本章ではMapping定義を最初から作成して利用します。
...
詳細は後ほど説明しますが、アナライザを適用することでそのフィールドに対し高度な検索を行うことができます。一方でkeyword型はアナライザが適用されないため、完全一致での検索が求められます。
...
作成されたIndexを確認します。@<list>{elasticsearch-list00}のエンドポイントから指定したIndex/TypeのMapping定義を確認できます。
...
また@<list>{elasticsearch-list08}のように、クライアント作成時にオプションを付与して試してください（以降のサンプルでも同様です）。
...
#@# ここリスト抜けてませんか？（「・・・クライアントのオブジェクトを作成します。」に対応するもの）（山城）

このクライアントオブジェクトを通じてElasticsearchを操作していきます。
クライアントの作成時に次の2つのオプションを指定しています。
特にSetSniffはElasticsearchのDockerコンテナへ接続する際に必要となる設定です。

#@# ２つのオプション、の列挙、あるいは紹介するリストが抜けていませんか？（山城）
...
IDの振り方には登録時にクライアント側で設定するか、Elasticsearch側でランダムに振ってもらうかの二種類があります。
 ...
 * Term Query
 ** 指定した文字列での検索をおこないますが、Match Queryとは違い検索指定文字列がAnalyzeされません。たとえば、タグ検索のように指定した文字列で完全一致させたドキュメントを探したい時などはTerm Queryを利用するといったケースです。
 * Bool Query
...
ここでAnalyzerについて簡単に説明します。Analyzerの設定は全文検索処理の要です。そのため、設定内容も盛り沢山ですし、自然言語処理の知識も必要となってくるため、ここではあくまでその触りだけを説明します。

この本をきっかけにElasticsearchにもっと興味を持っていただけた方は、Analyzerを深掘りしてみてください。


Analyzerは次の要素から構成されています。これらを組み合わせることで、より柔軟な検索をおこなうためのインデックスを作成することが可能です。

 * Tokenizer
 ** ドキュメントをどのようにトークン分割するかを定義します。トークン分割にはさまざまな方法があり、有名なものだと形態素解析やN-Gramなどがあります。Tokenizerにより分割されたトークンをもとに検索文字列との比較がおこなわれます。各Analyzerは1つのTokenizerをもつことができます。
 * Character filters
 ** Tokenizerによるトークン分割がされる前に施す処理を定義します。たとえば検索文字列のゆらぎを吸収するために、アルファベットの大文字・小文字を全て小文字に変換したり、カタカナの全角・半角を全て半角に統一したりといった処理をトークン分割の前処理として実施します。
 * Token filters
 ** Tokenizerによるトークン分割がされた後に施す処理を定義します。たとえば、形態素解析のように品詞をもとにトークン分割する場合、分割後のトークンから検索には不要と思われる助詞を取り除くといった処理が該当します。
...
Analyzerの設定はMapping定義のanalysisでおこないます。tokenizerでトークン分割の方法を設定し、analyzerで設定したtoknenizerと各filter群を組み合わせてひとつのAnalyzerを作ります。
本書では@<table>{analyzer}の設定でAnalyzerを設定しました。
...
意図したとおりのドキュメントを取得することができました！では、この検索結果はどのように導かれたのでしょうか。
...
この中に「テスト」というトークンが含まれているために意図どおりヒットしたというわけです。
...
そのため、たとえばタグ情報など指定した検索文字列と完全に一致させて検索をさせたい際に利用します。
...
Multi Fields機能を利用することで、ひとつのフィールドに対して異なるデータ型やAnalyze設定を指定することができます。
...
userフィールドのtypeにmulti_fieldを指定しています。次のようにフィールドを指定して操作することができます。

 * user： type textが適用されているuserフィールドにアクセスします
 * user.keyword：type keywordが適用されうるフィールドにアクセスします

ドキュメントを登録する際にはこれまでどおりuserフィールドを明示して登録するだけです。

たとえばMatchQueryの場合、次のようになります。
...
//list[elasticsearch-list178][user(type keyword)に対する検索]{
/* 省略 */
