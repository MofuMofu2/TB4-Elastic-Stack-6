...
監視やデータ分析をする際、テキストのデータの傾向を
テキストのまま分析するのは辛いものです。
この章では、この書籍を制作する際のgit commitがいつどのくらい行われているかについて分析し、可視化してみます。
...
あれ？出力内容が減っていますね。@<code>{--oneline}オプションをつけると、コミットのハッシュ値とコミットログ（1行目）しか出力されません。
これは少し不便です。
...
このままでは見づらいですね。
というわけで、Kibanaでこのコミットログをグラフにしてみたいと思います。

== Gitのコミットログをファイルに出力して、データの準備をする

というわけで、GitのコミットログをKibanaで閲覧してみましょう。まずはGitのコミットログをファイルに出力します。
...
どうでしょう？明らかにみんな締め切り近くに駆け込みコミット型なのがわかりました……ではなくて、大量のデータを分析するのに
Kibanaは結構便利だということが分かっていただけたでしょうか？

なるべく元データをLogstashで取得しやすい形に加工しておくことと、データの特性をちゃんと見て
グラフを作成していくのがポイントです。

データのパースにはそれなりのリソースを消費しますから、なるべくならデータはJSONなどの階層がある形式にしておきたいものです。
