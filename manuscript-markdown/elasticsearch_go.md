...
<!-- Mofu 絶賛開発中なのにメンテナンスがされていない、というのは矛盾している気が…？ -->
...
<!-- Mofu 開発が活発なのは公式クライアントも一緒？なので、もっと別の理由があると説得力があります -->

それではライブラリをインストールしてみましょう。
今回はgo getでインストールしますが、実際のプロダクト利用時はdepなどのパッケージ管理ツールの利用をおすすめします。

<!-- Mofu これはどこの環境にインストールしますか？ローカル/Dockerコンテナどちらか、この書き方だと一瞬迷います。 -->
...
<!-- Mofu ここはめんどくさがらずに各用語の解説・定義をしてあげると良いと思います。ESのハードルの高さはここの概念にあると個人的に思います -->
...
<!-- Mofu ここは図か何かを利用して新旧比較ができるとわかりやすそうです。 -->
...
<!-- Mofu 本を読んでいる人の中にはES入門者もいるかもしれないので、多いです、くらいの表現にとどめておく方が良いです。 -->

また、Mapping定義を作成することにより、各フィールド単位でより細かな検索設定をおこなえるため、本章では動的Mappingは利用せず、Mapping定義を作成し利用します。

<!-- Mofu 動的MappingとMapping定義の違いについて説明があると良いと思います。 -->
...
<!-- Mofu ゴール設定がここで記述されていた方がわかりやすいです。今回は**することを目標とします。と先にあった方が、後の文を読むときも先が見えて分かりやすさが上がります。 -->
...
#### ドキュメントの登録
まずは単一のドキュメントを登録します。
Elasticsearchは登録されたドキュメントに対してドキュメントIDと呼ばれるドキュメントを一意に識別するためのIDを付与します。
...
// Mofu どのように使い分けたら良いか説明があるとうれしいです。特徴が表かなにかにまとまっていて、ユースケースがあるとわかりやすいかも。
...
この本をきっかけにElasticsearchにもっと興味を持っていただけた方はAnalyzerについて詳しく調べてみてください。

#### Analyzerの基本
Analyzerは以下の要素から構成されています。これらを組み合わせることで、より柔軟な検索のためのインデックスを作成できます。
* Tokenizer
...
// Mofu トークン分割とは？という説明を入れた方がよいと思います。
...
// Mofu 具体的なデータを用いた事例を追記するとよりわかりやすくなると思います。
...
// Mofu Analyzer=アナライザ名でしょうか？表記は統一した方がわかりやすいです。
...
// Mofuコマンドの意味の説明があった方が良いです。基本の章で説明した方がよいでしょうか？

これで準備が整いました！それではここの詳細にうつっていきましょう。

// Mofu ここの詳細＝どこの詳細でしょうか？こそあど言葉はなるべく排除した方が良いです。
...
// Mofu エラーハンドリングの利点が説明されていると良いと思います。
